...
#define EXPERIMENT 0 // use this for the experimentation in this
...
// =============== Helper Functions ===============
// Provided below is a helper function which can sort an array by
//insertion sort.
// Name: insertionSort
// Input(s):
...
//that we will sort.
// (2) 'l' and 'r' are integers, which are the first index and
//the last index of 'arr' respectively.
// Output: No value is returned, but 'arr' should be modified to
//store a sorted array of numbers.
...
// (3) 'l', 'm' and 'r' are integers, which are the first
//index, the middle and the last index of 'array' respectively.
// Output: No value is returned, but 'array' should be modified to
//store a sorted array of numbers.
...
// This is the start of some 'contiguous block of memory'
//that we will sort.
// (2) 'size' tells us how big the array of data is we are
//sorting.
// Output: No value is returned, but 'arr' should be modified to
//store a sorted array of numbers.
...
// Start merging from size RUN. It will merge to size 2*RUN,
//then 4*RUN, and so on and so forth.
// 'l', 'm' and 'r' are the left, middle and right index of
//two contiguous subarrays.
...
// Input(s):
// (1) 'arr' is a pointer to an integer address.
// This is the start of some 'contiguous block of
//memory'.
