...
{
    for(j=1; j<2*n; j++)//Atribuição(j =inicia a 1); Comparação(j é menor que 2*n, se sim continua, se não finaliza o for); Incrementa(j recebe soma + 1).
    {
        s=s+1;//Atribuição(s =recebe valor dele mesmo e soma mais 1).
    }
}
